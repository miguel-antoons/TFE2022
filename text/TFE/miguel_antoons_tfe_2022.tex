...
%! add that the where the files are stored
%! image showing large spectrogram
...
%! reference annexe for scipy and numpy + comparaisons
...
\\
Une fois qu'on a les valeurs du fichier WAV, on calcule d'abord la moyenne de la densité spectrale de puissance, ou dsp, du bruit.
La dsp est une fonction qui représente la répartition des puissances en fonction de la fréquence d'un signal.
La moyenne de cette fonction représente la puissance moyenne par Hz d'un signal.\\
\\
...
Le code permettant de calculer la moyenne de la dsp du bruit est affiché à la figure \ref{fig:psd-code}.
...
Ensuite, on calcule la dsp en multipliant le vecteur S par son conjugué, ce qui nous donne son carré et donc la puissance pour chaque fréquence de l'axe x.
Enfin, on fait la moyenne du vecteur et on divise cette moyenne par la résolution fréquentielle ce qui nous donne une moyenne par Hz du bruit.\\
\\
%* consider adding a code sample for calculating calibrator psd
...
La fréquence retrouvée nous permet alors de calculer la dsp moyenne du signal.
Il y a ici deux différences par rapport au calcul de la dsp moyenne du bruit.
...
La deuxième est qu'une fois la dsp moyenne du signal calibreur est calculée, on y soustraire la dsp moyenne du bruit afin de disposer d'une estimation plus précise.\\
\\
...
Les deux valeurs de dsp moyenne sont enregistrées dans la base de données du projet BRAMS et sont donc réutilisables.
...
Ces actions sont faites à chaque fois qu'une nouvelle valeur de dsp moyenne est générée.\\
\\
Afin de détecter une grande variation dans les données, le programme se base sur l'ensemble des dsp moyennes des deux semaines précédent la nouvelle valeur de dsp.
À partir de cet ensemble, il faut définir une frontière haute et une frontière basse.
Si la nouvelle dsp moyenne se trouve en dehors d'une frontière, il sera considéré comme anormal et les scientifiques du projet BRAMS seront avertis.\\
\\
...
\\
Il faut savoir que les deux paramètres de la fonction affichée à la figure \ref{fig:detection-code} , \textbf{y\_data} et \textbf{current\_value}, sont l'ensemble des dsp moyennes des deux semaines précédent la nouvelle valeur de dsp et la nouvelle valeur de dsp.
%! exposant chars
...
En plus de la détection automatique d'une anomalie, une interface graphique permettant de visualiser l'évolution de la dsp moyenne du signal calibreur et du bruit a été développée.
...
Ceci est particulièrement visible sur la figure \ref{fig:BEHUMA4-variations} qui représente la dsp moyenne du bruit pour la station BEHUMA4 du 1er mars 2022 au 30 avril 2022.\\
\\
...
Cependant, ce code est temporaire et est vouée à disparaitre pour une solution plus efficace qui est en cours de développement par les membres du projet BRAMS.
Actuellement pour détecter les météores, les étapes suivantes sont exécutés par le code sur le spectrogramme :

\begin{enumerate}
    \item L'amplification des objets ressemblants à des météores par un filtre.
          %! show kernel in annexe
          Ce filtre a été développé spécialement pour ce but.
...
\begin{enumerate}
    ...
    \item La distance entre la station où est détecté l'écho et la station reçue en entrée.
\end{enumerate}
...
% \section{Kernel de Convolution Permettant d'Amplifier les Signaux de Météores}
